...
prepareIntegration = function(finalList, cc.use.genes, assay="RNA", nfeatures.variable = 3000, nfeatures.scale=3000, scale.regress=c('percent.rp', 'percent.mt', "nCount_RNA","S.Score", "G2M.Score"), normalize=TRUE, findvariable=TRUE, run.parallel=TRUE, npcs=50)
{
    ...
    objlist <- lapply(X = objlist, FUN = function(x) {
        ...
        x <- ScaleData(x, features = features, verbose = TRUE, assay=assay, vars.to.regress = scale.regress)
        x <- RunPCA(x, npcs=npcs, verbose = FALSE, reduction.name="pca", assay=assay)
...
#' @export
#'
preprocessIntegrated = function(obj.in, useAssay, inname, do.scale=T, num.pcs=50, resolution=0.5, plot.reduction="umap", dim.reduction="umap", with.hto=TRUE, run.parallel=TRUE, run.umap_neighbors=TRUE, clusters.graph.name=NULL, raster=TRUE)
{
  ...
  p=Seurat::DimPlot(obj.in, pt.size = 0.001, label=T, reduction = plot.reduction, raster=raster)
  ...
  p=Seurat::DimPlot(obj.in, pt.size = 0.001, label=T, split.by="orig_project", reduction = plot.reduction, ncol=2, raster=raster)
  ...
  if (with.hto)
  {
    tryCatch({
      ...
      p=Seurat::DimPlot(obj.in, split.by="libraryHTO", pt.size = 0.001, label=T, reduction = plot.reduction, ncol=3, raster=raster)
...
save_plot = function(plotobj, outname, fig.width, fig.height, save.data=TRUE, draw.obj=FALSE)
{
...
makeComplexExprHeatmap = function( obj.in, plot_gois, group.by="idents", use.scaled.data=TRUE, include_all_clusters=FALSE, title=NULL, scale.by="GLOBAL", scale.limits=NULL, log.expression=TRUE, ignore.missing=FALSE, rotateMatrix=FALSE)
{
...
for (gname in names(plot_gois))
{
    ...
    if (ignore.missing)
    {
      clusters = intersect(clusters, colnames(avgexp))
      genes = intersect(genes, rownames(avgexp))
    }
...
if (!is.null(scale.by) && (scale.by=="ALL"))
{
  ...
  p = Heatmap(mat, rect_gp = gpar(col = "white", lwd = 2), column_title=paste(title, sep="\n"), cluster_rows = FALSE,row_order = rownames(mat), column_order = colnames(mat), heatmap_legend_param = list(title = valueTitle))
...
HighlightedDimPlot = function(obj.in, highlightedClusters, group.by="idents", reduction=NULL, bgColour="#808080")
{
...
colorGroupDF$group = as.numeric(colorGroupDF$group)

if (is.factor(obj.in@meta.data[, group.by]))
{
  actualNames = levels(obj.in@meta.data[, group.by])
} else {
  actualNames = unique(obj.in@meta.data[, group.by])
}

colorGroupDF$group = actualNames[ colorGroupDF$group ]
colorGroupDF[!(colorGroupDF$group %in% highlightedClusters),]$colour = bgColour
...
makeBarCountPlot = function( seuratObj, outpath, select_by, group_by, size.text = 10, repel.direction="x", max.overlaps=10, fig.height=16, plot.summary=FALSE, plot.labels=TRUE, colors=NULL)
{
...
if (!plot.summary)
{
  cbc_cname = cbc_cname[cbc_cname$variable != "all",]
}

plot_distribution_celltype = cbc_cname %>% ggplot(aes(variable_factors, value, fill=cluster)) + geom_col(width=0.75)

if (!is.null(colors))
{
  plot_distribution_celltype = plot_distribution_celltype + scale_fill_manual(values=colors, breaks=names(colors))
}

if (plot.labels)
{

plot_distribution_celltype = plot_distribution_celltype + geom_text_repel(aes(label = paste(round(value, digits = 1))), max.overlaps=max.overlaps, size=size.text, position = position_stack(vjust = .5), direction =repel.direction, segment.size  = 1, segment.color = "black", min.segment.length=0, arrow = arrow(length = unit(0.015, "cm"), angle = 0, type = "closed", ends = "first"), force=2)

}
plot_distribution_celltype = plot_distribution_celltype + ylab("Percentage")+xlab("Condition")+
  theme(
...
# DT now only contains non-zero entries from expTable!
...
compareClusters = function(scdata, cellsID1, cellsID2, suffix1, suffix2, prefix="cluster", test="MAST", assay="RNA", outfolder="./", fcCutoff=0.25, all=FALSE, heatmap.plot=FALSE, heatmap.pcutoff=0.05, heatmap.addgenes=NULL)
{
    ...
    if (length(cellsID1) < 3)
    {
      print("!!! Cell group 1 has fewer than 3 cells")
      return(NULL)
    }
    if (length(cellsID2) < 3)
    {
      print("!!! Cell group 2 has fewer than 3 cells")
      return(NULL)
    }
    ...
    if (heatmap.plot)
    {
      ...
      if (length(genes.interest) > 0)
      {
        ...
        obj.rel = ScaleData(obj.rel, features=genes.interest, verbose=FALSE, assay=assay)

        p=DoHeatmap(obj.rel, genes.interest, group.by="heatmap_annot", assay=assay)+ scale_fill_gradientn(colors = scaleColors)
...
makeSideBySideDotPlot = function(scobj, plotElems, featureGenes = c(""), group.by="cellnames_manual", col.min = -3, col.max = 3, cols = c("grey", "blue"), title="", scaled=T, scale.by="ALL", rotate.x=F, abundance.perelem=FALSE)
{

  print("SBS DP")
  print(scale.by)

  stopifnot(is.null(scale.by) ||scale.by %in% c("FEATURE", "ALL", "GLOBAL"))
  ...
  for (plotName in names(plotElems))
  {
    ...
    scDf$Var1 = paste("c ", as.character(scDf$Var1))
    ...
    plotElem_orig$data$plot_condition = plotName
  ...
  for (plotName in names(plot_orig_list))
  {
    ...
    plot_orig_list[[plotName]]$data$id = paste("c ", as.character(plot_orig_list[[plotName]]$data$id))

    plot_orig_list[[plotName]]$data$id = factor(plot_orig_list[[plotName]]$data$id, levels = mixedsort(as.character(unique(plot_orig_list[[plotName]]$data$id))))
    plot_orig_list[[plotName]]$data$idn= as.numeric(plot_orig_list[[plotName]]$data$id)

    plot_orig_list[[plotName]]$data$features.plot = as.character(plot_orig_list[[plotName]]$data$features.plot)

    print(plotName)
    print(plot_orig_list[[plotName]])
  }
  ...
  {
    plotElem_orig = plot_orig_list[[plotName]]
    max.pct.exp = max(c(max.pct.exp, plotElem_orig$data$pct.exp))
  ...
  max.pct.exp = ceiling(max.pct.exp)
  ...
  print("Merging single plots")
  
  print(paste(min.pct.exp, max.pct.exp))
  ...
  for (plotName in names(plot_orig_list))
  {
    ...
    plotElem_orig <- ggplot(pData) +
      scale_x_continuous(breaks=pData$featuren, labels=pData$features.plot) +
      scale_y_continuous(breaks=pData$idn, labels=pData$id, limits = c(min(pData$idn)-0.6, max(pData$idn)+0.6))+
      geom_rect(aes(xmin=featuren-.5, xmax=featuren+.5, ymin = idn-0.5, ymax = idn+0.5, fill=percn), alpha = 0.4, linetype="blank") +
      scale_fill_distiller(palette='Spectral', limits = fillLimits)+
      geom_point(aes(x=featuren, y=idn, colour = avg.exp.scaled2, size = pct.exp)) +
      scale_size_continuous(aes(pct.exp), limits=c(min.pct.exp, max.pct.exp))+
      scale_color_gradient2(limits=c(col.min, col.max), low = cols[1], mid=cols[2], high = cols[3])+
      guides(color=guide_colourbar(title=title.expression, order = 1),
      size=guide_legend(title="Percent Expressing", order = 2),
      fill=guide_colourbar(title=title.cellabundance, order = 3))
...
splitFeaturePlot = function(obj, feature, split.by, title, filename,limits=c(-1,1), reduction="umap", ncol=NULL, low="lightgrey", high="blue", mid="white", mirrorLimits=TRUE)
{
  ...
  prow = combine_plot_grid_list(plotlist=pds, label_x = "a", ncol=ncol, align="hv")
  fplot= combine_plot_grid_list(plotlist=list(title, prow), ncol=1, rel_heights=c(0.1, 1))

  if (!is.null(filename))
  {
    save_plot(fplot, filename, fig.width=ncol *6 + 2, fig.height=nrow*6)
...
plotPValueViolentBoxPlot = function( obj.sc, feature, group.by, split.by=NULL, split.values=NULL, dsrCols=NULL, onelineLabel=FALSE, dot.size=0, min.threshold=3, yStepIncrease=0.25, override=FALSE, boxplot_grey="grey")
{
  ...
  bxp = bxp +
        geom_boxplot(aes_string(fill=split.by),color=boxplot_grey, alpha=0.4, position =position_dodge(width = 0.9)) +
...
makePieTrieCounts = function( seuratObj, outpath, select_by, group_by, size.text = 10, repel.direction="x", max.overlaps=10, fig.height=16, pie_width=6, use.palette=NULL, clusterPrefix="Cluster")
{
...
if (is.factor(seuratObj@meta.data[,select_by]))
{
  cbc_cname$cluster = factor(cbc_cname$cluster, levels=levels(seuratObj@meta.data[,select_by]))
} else {
  cbc_cname$cluster = factor(cbc_cname$cluster, levels=unique(seuratObj@meta.data[,select_by]))
}


cbc_cname$total = 100
cbc_cname$label = paste0(clusterPrefix, cbc_cname$cluster, " (", round(cbc_cname$value, 2), "% )")
cbc_cname$perc = paste0(round(cbc_cname$value, 2), "%")

print("Cluster Levels")
print(levels(cbc_cname$cluster))
...
plotvars = unique(cbc_cname$variable)
  ...
  for (plotcond in plotvars)
  {
    ...
    subdf$axis_label=paste(clusterPrefix, subdf$cluster)
    data = subdf %>% mutate(ypos = cumsum(value)- 0.5*value, csum = rev(cumsum(rev(value))), axispos = value/2 + lead(csum, 1), axispos = if_else(is.na(axispos), value/2, axispos))
  ...
  p=ggplot(data, aes(x = "", y = value, fill = cluster)) +
    ...
    guides(fill = guide_legend(title = clusterPrefix)) +
    scale_y_continuous(breaks = data$axispos, labels = data$axis_label) +
    theme(axis.ticks = element_blank(),
          axis.title = element_blank(),
          axis.text = element_text(size = 15), 
          axis.text.x = element_text(margin = margin(t = 1, unit = "in")),
          legend.position = "none", # Removes the legend
          panel.background = element_rect(fill = "white"))+ ggtitle(plotcond)
  ...
  topLegend = combine_plot_grid_list(list(topPlot, legend_b), ncol=1, rel_heights=c(1.0, 0.2))

  numcols = length(condplots)
  use_width = numcols * pie_width
...
scatterAnalysisPlot = function(mat, gene1, gene2, logg1=TRUE, logg2=TRUE, formula=y ~ x, xlim.plot=NULL, ylim.plot=NULL)
{
  ...
  if (is.null(ylim.plot))
  {
    ...
    ylim.plot = c(lowerlimY, upperlimY)
  }

  p = ggplot(df, aes(x=.data[[gene1]], y=.data[[gene2]])) +
  geom_point() + 
  geom_text_repel(label=rownames(df), max.overlaps=20, direction="both", force=2)+stat_smooth(formula=formula, method = "lm")+
  stat_cor(r.accuracy = 0.01, label.y = 0.9*ylim.plot[2], size = 4, label.sep='\n') +
  stat_regline_equation(aes(label = ..eq.label..), size = 4, label.y = 0.8*ylim.plot[2]) + theme_classic()

  p = p + xlim(xlim.plot[1], xlim.plot[2])
  p = p + ylim(ylim.plot[1], ylim.plot[2])
